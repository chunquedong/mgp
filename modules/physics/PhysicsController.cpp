    ...
    case PhysicsCollisionShape::SHAPE_HEIGHTFIELD:
        {
            ...
            else
            {
                ...
                if (dynamic_cast<Terrain*>(node->getDrawable()) == NULL)
                    ...
                    collisionShape = createHeightfield(node, dynamic_cast<Terrain*>(node->getDrawable())->getHeightfield(), centerOfMassOffset);
            }
        }
        break;
...
PhysicsCollisionShape* PhysicsController::createMesh(Mesh* mesh, const Vector3& scale, bool dynamic)
{
    ...
    char* vertexData = (char*)mesh->getVertexBuffer()->_data;
    if (vertexData == NULL)
    {
    ...
    {
        v.set(*((float*)&vertexData[i * vertexStride + 0 * sizeof(float) + vertexOffset]),
              *((float*)&vertexData[i * vertexStride + 1 * sizeof(float) + vertexOffset]),
              *((float*)&vertexData[i * vertexStride + 2 * sizeof(float) + vertexOffset]));
        v *= m;

        shapeMeshData->vertexData[i * 3] = v.x;
        shapeMeshData->vertexData[i * 3+1] = v.y;
        shapeMeshData->vertexData[i * 3+2] = v.z;
    ...
    else
    {
        ...
        case Mesh::INDEX32:
            ...
            break;
        default:
            GP_ERROR("Unsupported index format (%d).", mesh->getIndexFormat());
        ...
        if (mesh->getIndexBuffer()->_dataSize > 0) {
            int bufsize = mesh->getIndexBuffer()->_dataSize;
            void* indexData = malloc(bufsize);
            memcpy(indexData, mesh->getIndexBuffer()->_data, bufsize);
            shapeMeshData->indexData = (unsigned char*)indexData;
        }
        else {
            // Generate index data for the mesh locally in the rigid body.
            unsigned int* indexData = (unsigned int*)malloc(sizeof(unsigned int) * vertexCount);
            for (unsigned int i = 0; i < vertexCount; i++)
            {
                indexData[i] = i;
            }
        ...
        else
        {
            ...
            indexedMesh.m_triangleIndexBase = shapeMeshData->indexData;
       ...
       btIDebugDraw::DBG_DrawContactPoints | btIDebugDraw::DBG_DrawWireframe), _meshBatch(NULL), _lineCount(0)
{
    ...
    GP_ASSERT(material.get() && material->getStateBlock());
    ...
    _meshBatch = MeshBatch::create(VertexFormat(elements, 2), Mesh::LINES, std::move(material), Mesh::INDEX16, 4096, 4096).take();
