    ...
    : _alSource(source), _buffer(buffer), _looped(false), _gain(1.0f), _pitch(1.0f), _node(NULL)
{
    ...
    AL_CHECK( alSourcefv(_alSource, AL_VELOCITY, velocity) );
}
...
UPtr<AudioSource> AudioSource::create(const char* url, bool streamed)
{
    ...
    if (pathStr.find(".audio") != std::string::npos)
    {
        ...
        if (properties.get() == NULL)
        {
            ...
            return UPtr<AudioSource>(NULL);
        }

        UPtr<AudioSource> audioSource = create((strlen(properties->getNamespace()) > 0) ? properties.get() : properties->getNextNamespace());
    ...
    return UPtr<AudioSource>(new AudioSource(buffer, alSource));
}

UPtr<AudioSource> AudioSource::create(Properties* properties)
{
    ...
    UPtr<AudioSource> audio = AudioSource::create(path.c_str(), streamed);
    if (audio.get() == NULL)
    {
...
{
    ALfloat svelocity[3];
    svelocity[0] = velocity.x;
    svelocity[1] = velocity.y;
    svelocity[2] = velocity.z;
    AL_CHECK( alSourcefv(_alSource, AL_VELOCITY, svelocity) );
    _velocity = velocity;
}
...
void AudioSource::transformChanged(Transform* transform, long cookie)
{
    if (_node)
    {
        ...
        s[1] = translation.y;
        s[2] = translation.z;
        AL_CHECK( alSourcefv(_alSource, AL_POSITION, s) );
    }
}
