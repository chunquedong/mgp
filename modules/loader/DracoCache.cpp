...
bool DracoMesh::getFaceIndices(cgltf_accessor* target) const {
    ...
    if (target->count != count) {
        printf("The glTF accessor wants %d indices, "
               "but the decoded Draco mesh has %d indices.\n", target->count, count);
        return false;
    }
    ...
    switch (target->component_type) {
        ...
        default:
            printf("Unexpected component type for Draco indices.\n");
            return false;
    }
    return true;
}
...
bool DracoMesh::getVertexAttributes(uint32_t attributeId, cgltf_accessor* target) const {
    ...
    if (!attr) {
        printf("Unknown Draco point attribute.\n");
        return false;
    }
    ...
    if (target->count != count) {
        printf("The glTF accessor wants %d vertices, "
            "but the decoded Draco mesh has %d vertices.\n", target->count, count);
    ...
    switch (target->component_type) {
        ...
        default:
            printf("Unexpected component type for Draco vertices.\n");
            break;
    }
