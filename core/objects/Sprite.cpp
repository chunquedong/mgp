                       ...
                       const Rectangle& source, unsigned int frameCount, ShaderProgram* effect)
{
    ...
    SpriteBatch* batch = SpriteBatch::create(imagePath, effect).take();
...
unsigned int Sprite::draw(RenderInfo* view)
{
    ...
    _batch->start();
    _batch->drawImageRotated(position, _frames[_frameIndex], scale, Vector4(_color.x, _color.y, _color.z, _color.w * _opacity),
...
UPtr<Drawable> Sprite::clone(NodeCloneContext& context)
{
    ...
    return UPtr<Drawable>(spriteClone);
}
