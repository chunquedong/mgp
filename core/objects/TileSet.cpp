                         ...
                         unsigned int rowCount, unsigned int columnCount)
{
    ...
    SpriteBatch* batch = SpriteBatch::create(imagePath).take();
...
unsigned int TileSet::draw(RenderInfo* view)
{
    ...
    if (_node && _node->getScene())
    {
        ...
        if (activeCamera)
        {
            Node* cameraNode = activeCamera->getNode();
            if (cameraNode)
            {
                // Scene projection
    ...
    for (unsigned int row = 0; row < _rowCount; row++)
    {
        for (unsigned int col = 0; col < _columnCount; col++)
        {
                ...
                _tiles[row * _columnCount + col].y >= 0)
            {
                ...
                _batch->drawImageRotated(position, source, scale, Vector4(_color.x, _color.y, _color.z, _color.w * _opacity),
                             Vector2(0.5f, 0.5f), 0);
            }
...
UPtr<Drawable> TileSet::clone(NodeCloneContext& context)
{
    ...
    return UPtr<Drawable>(tilesetClone);
}
