...
AnimationTarget::~AnimationTarget()
{
    if (_animationChannels)
    {
        ...
        {
            AnimationChannel* channel = (*itr);
            GP_ASSERT(channel);
            GP_ASSERT(channel->getAnimation());
            channel->getAnimation()->removeChannel(channel);
            SAFE_DELETE(channel);
            itr++;
        }
...
{
    UPtr<Properties> p = Properties::create(url);
    GP_ASSERT(p.get());

    Animation* animation = createAnimation(id, (strlen(p->getNamespace()) > 0) ? p.get() : p->getNextNamespace());
...
void AnimationTarget::destroyAnimation(const char* id)
{
    ...
    AnimationChannel* channel = getChannel(id);
    if (channel == NULL)
        return;
...
Animation* AnimationTarget::getAnimation(const char* id) const
{
    if (_animationChannels)
    {
        ...
        if (id == NULL)
            return (*itr)->getAnimation();

        AnimationChannel* channel = NULL;
        for (; itr != _animationChannels->end(); itr++)
        {
            channel = (AnimationChannel*)(*itr);
            GP_ASSERT(channel);
            GP_ASSERT(channel->getAnimation());
            if (channel->getAnimation()->_id.compare(id) == 0)
            {
                return channel->getAnimation();
            }
        }
    }
...
void AnimationTarget::addChannel(AnimationChannel* channel)
{
    if (_animationChannels == NULL)
        _animationChannels = new std::vector<AnimationChannel*>;

    GP_ASSERT(channel);
    _animationChannels->push_back(channel);
}

void AnimationTarget::removeChannel(AnimationChannel* channel)
{
    if (_animationChannels)
    {
        std::vector<AnimationChannel*>::iterator itr = _animationChannels->begin();
...
AnimationChannel* AnimationTarget::getChannel(const char* id) const
{
    if (_animationChannels)
    {
        std::vector<AnimationChannel*>::iterator itr = _animationChannels->begin();
...
void AnimationTarget::cloneInto(AnimationTarget* target, NodeCloneContext &context) const
{
    if (_animationChannels)
    {
        for (std::vector<AnimationChannel*>::const_iterator it = _animationChannels->begin(); it != _animationChannels->end(); ++it)
        {
            KeyframeChannel* channel = dynamic_cast<KeyframeChannel*>(*it);
            if (!channel) continue;
            GP_ASSERT(channel);
            GP_ASSERT(channel->getAnimation());

            Animation* animation = context.findClonedAnimation(channel->getAnimation());
            if (animation != NULL)
            {
                AnimationChannel* channelCopy = new KeyframeChannel(*channel, animation, target);
                animation->addChannel(channelCopy);
            }
            else
            {
                // Clone the animation and register it with the context so that it only gets cloned once.
                animation = channel->getAnimation()->clone(channel, target);
                context.registerClonedAnimation(channel->getAnimation(), animation);
            }
        }
    }
}
