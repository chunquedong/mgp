...
UPtr<Properties> Properties::create(const char* url)
{
    if (!url || strlen(url) == 0)
    {
        return UPtr<Properties>(NULL);
    }
    ...
    UPtr<Stream> stream(FileSystem::open(fileString.c_str()));
    if (stream.get() == NULL)
    {
